...
/*
HRESULT Image::CreateDeviceResources(HWND hWnd, ID2D1HwndRenderTarget* m_pRT, ID2D1Bitmap* m_pD2DBitmap)
{
	HRESULT hr = S_OK;
...
void Image::Draw(HWND hwnd, int xPos, int yPos, int size, ID2D1HwndRenderTarget* m_pRT)
{
	m_pRT->SetTransform(D2D1::Matrix3x2F::Identity());
	...
	if (m_pD2DBitmap)
	{
		m_pRT->DrawBitmap(m_pD2DBitmap, D2D1::RectF(xPos, yPos, xPos + size, yPos + size));
	}
